package com.nx.nxfs.util;

import com.nx.nxfs.distributed.BlockPlacementManager;
import com.nx.nxfs.distributed.PropertiesUtil;
import com.nx.nxfs.distributed.Server;
import com.nx.nxfs.distributed.ServerLoader;
...
public class IOUtils {
    ...
    public static void copyFileToDFS(String inputFile, String outputDir) {
        ...
        for (int i = 0; i < trueBlocks; i++) {
            ...
            perfectServerPath = perfectServerPath
                    .replace("\\", "/")
                    .replace("//", "/");
            ...
            copyReplicatioin(chosedServers, perfectServerPath, outputDir, blockID);
    ...
    private static void copyReplicatioin(List<Server> chosedServers, String perfectServerPath,
                                         String outputDir, String blockID) {
        if(chosedServers.size() == 1){
            return;
        }

        for (int i = 1; i < chosedServers.size(); i++) {

            String inputPath = perfectServerPath;
            String serverPath = chosedServers.get(i).getServerPath();
